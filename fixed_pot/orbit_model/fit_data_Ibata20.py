...
def orbit_model(alpha,delta,distance,mu_alpha,mu_delta,v_los):
    ...
    return phi_1, phi_2, d_hel, v_hel, mu_ra, mu_dec, y[0], y[1], y[2]
...
phi_1,phi_2,d_hel,v_hel,mu_1,mu_2, x,y,z = orbit_model(154.43815737978053, 41.352611518783334, 9.127975838807465, -6.135527680412597, -9.275558167001593, -63.00000001)  
...
plt.tight_layout()
plt.show()
fig.savefig("plots/orbit_xz_fit_data_Ibata2rar.png")
...
def chi2(w_0):
    phi_1,phi_2,d_hel,v_hel,mu_ra,mu_dec, x,y,z = orbit_model(w_0[0],w_0[1],w_0[2],w_0[3],w_0[4],w_0[5]) 
    ...
    mu_ra_spl  = interp1d(phi_1,mu_ra,kind='cubic')
    mu_dec_spl  = interp1d(phi_1,mu_dec,kind='cubic')
    ...
    def mu_ra_wrap(x):
        ...
        for i in range(0,len(x)):
            if(x[i]<phi_1_min):
                array[i]= mu_ra_spl(phi_1_min)
            elif(x[i]>phi_1_max):
                array[i]= mu_ra_spl(phi_1_max)
            else:
                array[i]= mu_ra_spl(x[i])
        return array
    ...
    def mu_dec_wrap(x):
        ...
        for i in range(0,len(x)):
            if(x[i]<phi_1_min):
                array[i]= mu_dec_spl(phi_1_min)
            elif(x[i]>phi_1_max):
                array[i]= mu_dec_spl(phi_1_max)
            else:
                array[i]= mu_dec_spl(x[i])
        return array
