...
Optimization to fit the best RAR parameters with MCMC (emcee package).
...
#
# Rotation curve from Sofue 20.
v_Sof20 = pd.read_csv('vel_Sofue20.txt', sep=" ")
r_data = v_Sof20['Radius']
v_data = v_Sof20['Velocity']
dv_data = v_Sof20['Error']
...
def log_probability(param, ener_f, beta_0, ic, bounds):
    ...
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(param, ener_f, beta_0, ic)
...
# Parameters
param_file = 'param_fit_mcmc_pot-slice-rot_from_IbataPolysGaiaDR2-data.txt'
backend_file = 'backend_fit_mcmc_pot-slice-rot.h5'
inparam_file = 'inparam.txt'
r_sun, ener_f, beta_0, weight, bool_mcmc = np.loadtxt(inparam_file)
print('inparams:')
...
plt.legend()
plt.tight_layout()
fig.savefig("orbit_mcmc_pot-slice-rot_from_IbataPolysGaiaDR2-data.png")
# plt.show()
...
# plt.tight_layout()
fig.savefig("sky_mcmc_pot-slice-rot-rot_from_IbataPolysGaiaDR2-data.png")
# plt.show()
...
plt.savefig('rotation_curve_mcmc_beta0_1.25e-5.png')
# plt.show()
