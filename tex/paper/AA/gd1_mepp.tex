...
% 5 {} token are mandatory
  ...
  % {} leave it empty if necessary
   ...
   deduced from first principles by means of the maximization of the coarse-grained entropy for self-gravitating particles, the MEPP, after Maximum Entropy Production Principle. One example of a MEPP
   is the family of Relativistic Fermionic King  (RFK) distributions which have a rich
  ...
  % aims heading (mandatory)
   {In this work we attempt to model the GD-1 stellar stream using a spherical RFK distribution which, at the same time, is in synthony with previous fits to the S-stars.}
  ...
  % results heading (mandatory)
   ...
   mass going from $56~\rm{keV}/c^2$ to $360~\rm{keV}/c^2$, respectively corresponding to core radii going from $\sim 10^3$ to 5 Schwarzschild radii. For smaller and larger values of the fermion mass, there is no solution that simultaneously fits the GD-1 stream and the S-stars. The solutions have a virial radius of 28 kpc and a virial mass of $1.4\times10^{11} M_\odot$, the latter being at a $2\sigma$ distance from previous Milky Way DM halo mass estimates using the Sagittarius stream. We do not assume the velocity of the local standard of rest ($v_\mathrm{lsr}$) and the result gives $v_\mathrm{lsr}\approx244$ km/s, in agreement with recent independent estimates. }
  % conclusions heading (optional), leave it empty if necessary
  {This paper provides further evidence of the fact that stellar streams constitute reliable measurement systems of the acceleration field of the Galaxy.}
...
In this paper we will model the formation of the GD-1 stellar stream inside a MW with a relativistic fermionic King dark matter core-halo distribution. In Sec.~\ref{sec:methodology} we explain the work methodology. In Sec.~\ref{sec:results} we present the results  and in Sec.~\ref{sec:conclusions} we give the conclusions.
...
by $m$ and $\beta_0$ that control the core of the distribution in the sense that
for given values of $m_{\rm{core}}$ (Sec.~\ref{sec:observables}) and $m$, it is possible to find a value of $\beta_0$ in agreement; with a very small influence of the other two parameters: $\theta_0$ and $W_0$. This implies a partial degeneracy between $m$ and $\beta_0$.
...
As a starting point for the parameter fit, we took the result from~\citet{2020A&A...641A..34B} who found that for $(m, \beta_0, \theta_0, W_0)=(56~\rm{keV}/c^2, 1.1977\times10^{-5}, 37.765, 66.3407)$
...
As a first step, we fixed $m=56~\rm{keV}/c^2$ and
...
$(\theta_0, \omega_0, \beta_0)\approx (36.0665,27.3471 , 1.25713\times10^{-5})$
 ...
 $m_c\approx 3.537\times 10^6 M_\odot$ which implies that there is room for improvement.
...
to $(\theta_0, \omega_0, \beta_0)\approx (36.0661, 27.3468,1.25769\times10^{-5})$ with a value of $\chi^2_{\rm{stream}}\approx 21.9585$ and  $\chi^2_{\rm{core}}\approx 1.344\times10^{-5}$ which corresponds to
 a much precise value of the core mass, $m_c\approx 3.5001\times 10^6  M_\odot$. It can be noticed the great sensitivity of the core mass depence on the model parameters because a change in the fifth significant digit of the parameters propagates to a change of in  the third significant digit of $m_c$. Nevertheless, both solutions above are astrophysically equivalent because~\citet{2021MNRAS.505L..64B} has prooved that core masses in the range $[3.5, 3.55]\times 10^6 M_\odot$ give good fits of the S-stars.
...
%
We would like to give a glimpse of the behaviour of the $\chi^2_{\rm{stream}}$ function so we have evaluated it for fixed values of $(m, \beta_0)=(56~\rm{keV}/c^2, 1.258\times10^{-5})$ in a bidimensional grid
...
obtaining $h(x)\approx 0.7989+0.7361x$, substracted this line to $\omega_0$ and made a plot of $\chi^2_{\rm{stream}}$ function for $(m, \beta_0)=(56~\rm{keV}/c^2, 1.258\times10^{-5})$ in the window $(\theta_0, \omega_0-h(\theta_0))\in[35, 37]\times[-0.011,0.011]$. The result can be seen in the contour plot of
...
\begin{figure}
   ...
   \caption{$\chi^2_{\rm{stream}}$ function for $(m, \beta_0)=(56~\rm{keV}/c^2, 1.258\times10^{-5})$ in
...
Our solution has a an average slope $s\approx-3.44~\rm{km~s}^{-1}~kpc^{-1}$, fitted for Galactocentric distances between 5 and 40 kpc, which is a bit larger in magnitutude than the value of $s=-2.18~\rm{km~s}^{-1}~kpc^{-1}$ measured by~\citet{2023arXiv230900048J}.
...
We would like to point out that there are recent model-dependent MW mass estimations performed by
by~\citet{2023arXiv230900048J} and~\citet{2023arXiv230312838O} which give even smaller values of the MW virial mass of approximately $1.99\times10^{11}M_\odot$ and $2.13\times10^{11}M_\odot$, respectively

Our fitted fermionic DM model has a density at the solar neighbourhood of
$\rho_{DM,\odot}=1.46\times10^7M_\odot~\rm{kpc}^{-3}=0.55~\rm{GeV}~\rm{cm}^{-3}/c^2$ which belongs to
the $2\sigma$ region of the estimation made by~\citet{Salucci2010}.
...
has been measured. This diameter corresponds to a shadow radius of $\sim 2.46$ Schwarzschild radius assuming
a black hole mass of $m_{\rm{bh}}=4.075\times10^6~M_\odot$.
In order to extend our solutions to other values of the fermion mass ($m$), we used the second optimization algorithm described in Sec.~\ref{sec:optimization} for $m=100, 200, 300, 360~\rm{keV}/c^2$. For each fermion mass we divided a given macroscopic orthohedron in parameter space in $20^3=8000$ smaller orthoedrons. Each subregion was optimized in a parallel distributed scheme, searching for those parameters that minimize $\chi^2_{\rm{full}}$ with the \texttt{NOMAD} algorithm. Afterwards, we selected the global minimum by comparing the results of each distributed process.
The result is that for all the fermion masses it is possible to find values of the other parameters in such a way that the GD-1 stream is fitted with the same quality as the initial ($m=56~\rm{keV}/c^2$) case. In fact, as shown in Fig.~\ref{fig:going_compact}, all the solutions have the same density profile in the halo region, while their difference is limited to the compactness of the core.
The values of the core radii of these solutions are approximately 1096, 232, 35, 10, 5 in units of  Shwarzschild radius, respectively for $m=100, 200, 300, 360~\rm{keV}/c^2$.
\begin{figure*}
   \centering
...
We have fitted the GD-1 stream with an orbit in a MW potential built by a relativistic fermionic King
DM distribution plus a set of fixed baryion distributions previously fitted by Pouliasis et al.
The general procedure was to fix the value of the fermion mass while floating the other three fermionic
parameters in order to fit simultaneously two astrophysical constraints: the stream observables and a
DM core mass of $3.5\times10^6M_\odot$, where the latter comes from previous fits of the S-star cluster at
the centre of the Galaxy. This allowed us to reproduce with high accuracy, for fermion masses ranging from 56 to 360 keV, the polynomials fitted by~\citet{Ibata_2020} that correspond to sky position, proper motion, photometric distance and radial velocity measurements. Note that the core radius
does not include all the core mass because it is defined as the radius in which the circular velocity reaches
its first maximum corresponding to a distance in which the core density is still falling.
We have fitted the stream track with a spherical fermionic DM distribution similarly to the fit performed by~\citet{2019MNRAS.486.2995M} with a triaxial generalization of the NFW distribution, obtaining agreement in both rotation curves at Galactocentric distances corresponding to the stream observables, i.e.
$11.5\leq D_g \leq 16.4$ (kpc). The slope of our Fermionic-MW model's rotation curve between 5 and 40 kpc is
$s\approx-3.44~\rm{km~s}^{-1}~kpc^{-1}$.

We have found a circular velocity at the sun's position of $v_c(R_\odot)\approx 244~\rm{km~s}^{-1}$ equal to the value obtained by~\citet{2019MNRAS.486.2995M}.
Our fermionic DM solution has a finite radius of $r_{\rm{DM,vir}}=27.4$~kpc and virial mass of
$m_{\rm{DM,vir}}=1.4\times10^{11}~M_\odot$, implying a total (DM+baryons) virial mass of the galaxy of $m_{\rm{vir}}= 2.4\times10^{11}~M_\odot$, which is at a distance of $2\sigma$ from the value reported in Table 3 of
\citet{2014MNRAS.445.3788G} for a radius of 50~kpc. The value obtained for the DM density at the solar neighbourhood is around $\rho_{DM,\odot}=1.46\times10^7M_\odot~\rm{kpc}^{-3}=0.55~\rm{GeV}~\rm{cm}^{-3}/c^2$ which belongs to the $2\sigma$ region of a previos estimate~\citep{Salucci2010}.
Finally, we have shown that it is possible to find a unidimensional family of solutions parametrized by the fermion mass, which have the same halo that fits the GD-1 stream but they differ in the compactness of the central core. For the limiting case studied here ($m=360$~keV) we obtain a core radius of $r_c\approx5$~Schwarzschild radius where the transformation to this distance unit was done assuming a black hole mass of $m_{\rm{bh}}=4.075\times10^6~M_\odot$.
...
In summary, the findings of this work support the idea that tidal streams are excellent probes of the
acceleration field of the Milky Way.
