...
In order to extend our solutions to other values of the fermion mass ($m$), we use the fact that for
the optimal solution for $m+\Delta m$ is close to the corresponding solution for $m$. This allows us to start with our solution for $m=56$ keV, increasing sequentially the the mass with $\Delta m=0.1$ keV, and fitting the other model parameters ($\theta_0, \omega_0, \beta_0$) at each step. In this case we used the second optimization algorithm described in Sec.~\ref{sec:optimization} for $56<m<370$ keV.
For all the fermion masses it is possible to find values of the other parameters in such a way that
the GD-1 stream is fitted as well as in our previously studied case. In fact, as will be shown in the following Fig.~\ref{fig:going_compact}, all the solutions have the same density profile in the halo region,
while their difference is the degree of compactness of the core.

\begin{figure*}
   \centering
   \includegraphics[width=0.9\textwidth]{figures/densities.png}
   \caption{.}
...
\begin{acknowledgements}
    ...
    MFM thanks Leandro Martínez and the \textsc{Julia} community for their
