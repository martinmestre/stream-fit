...
a Python~\citep{van1995python} script\footnote{
\url{https://github.com/martinmestre/stream-fit/blob/main/likelihood_grid/optim/model_def.py}
}
that makes use of the NumPy~\citep{2020SciPy-NMeth} and SciPy~\citep{harris2020array} libraries. We tried all the solvers available in the latter package but the only one that
...
we will explain how this initial condition was chosen for some simulations and fitted for others. The integrator used is a Runge-Kutta of order eight (\texttt{DOP853} called from SciPy's \texttt{solve\_ivp} function) with relative and absolute tolerance parameters respectively given by
...
The other algorithm is \texttt{LN\_NELDERMEAD}~\citep{NELDERMEAD,10.1093/comjnl/8.1.42,shere} from the NLopt~\citep{NLopt} package
for the Julia~\citep{bezanson2017julia} language.
...
\begin{acknowledgements}
    ...
    The figures presented where made with the AlgebraOfGraphics.jl\footnote{https://aog.makie.org/dev/} and Makie.jl~\citep{DanischKrumbiegel2021}
    packages.
