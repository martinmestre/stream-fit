...
\begin{eqnarray}
   ...
   \chi^2_{\eta} &=& \frac{1}{\sigma_\eta^2}\sum_{i=1}^{100} [\eta^{(i)}-\eta(\phi_1^{(i)})]^2,
\end{eqnarray}
where
$\eta$ represents the four corresponding independent variables ($\phi_2$, $D$, $v_h$, $\tilde{\mu}_\alpha$ and $\mu_\delta$) and $\sigma_\eta$ is an approximate (eye-guided) estimation of the average errors given by
$\sigma_{\phi_2}=0.5^\circ$,
$\sigma_{D}= 1.5~\rm{kpc}$,
$\sigma_{v_h} = 100~\rm{km/s}$,
$\sigma_{\tilde{\mu}_\alpha}= 4~\rm{mas/yr}$
and $\sigma_{\delta}= 4~\rm{mas/yr}$.
