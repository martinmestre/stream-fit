...
\end{eqnarray}
where $M(r)$ is the mass enclosed up to radius $r$, $\nu(r)$ is the metric exponent (we use the convention $g_{00}=e^{\nu(r)}$), $c$ is the speed of light, $h$ is the Planck constant and $m$ is the fermion mass. The phase-space function ($f$) is given by a Fermi-Dirac distribution with energy cutoff:
\begin{equation}
f(r,\epsilon)=
   \begin{cases}
...
It is not possible to integrate this equations from a radius exactly zero because the right hand side
of the system of differential equations are not well defined in that situation. Nevertheless, it is possible
to reach the following approximations for the initial conditions at a value $r_{\rm{min}}$ close to zero:
\begin{eqnarray}
