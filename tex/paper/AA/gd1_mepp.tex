...
found is given by approximately by $\alpha=149.4^{\circ}$, $\delta=36.8^{\circ}$, $D=7.9$ kpc,
$\tilde{\mu}_\alpha=-7.1$ mas/yr, $\mu_\delta=-12.5$ mas/yr and $v_h=-17.3$ km/s.
...
From previous works~\citep{arguelles_novel_2018,2022MNRAS.511L..35A} we know that the fermionic DM
model parameters can be approximately grouped in two sets with different functionality. One group is formed
by $\epsilon$ and $\beta_0$ that control the core of the distribution in the sense that
for given values of $m_{\rm{core}}$ (as defined in Sec.~\ref{sec:observables}) and $\epsilon$, it is possible to find a value of $\beta_0$ in agreement; with a very small influence of the other two parameters: $\theta_0$ and $W_0$. This implies a partial degeneracy between $\epsilon$ and $\beta_0$.
The other group is formed by $\theta_0$ and $W_0$, which have negligible effect on the core region but
determine the distribution behaviour in the halo region.
Then, we started with plausible fixed values for the pair $(\epsilon,\beta_0)=(56,1.25\times10^{5})$
that satisfy approximately the $m_{\rm core}=3.5\times 10^5 M_\odot$ contraint, together with
the fixed IC fitted for the NFW-MW model, and performed a genetic optimization in the windows:
$\theta_0\in (35, 40)$  $W_0-\theta_0$ (35, 40), (25, 30)
