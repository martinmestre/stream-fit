...
determine the distribution behaviour in the halo region. In fact, there is also an approximate
...
As a starting point for our three-step multiparameter search, we took the result from~\citet{2020A&A...641A..34B}, that found that for $(\epsilon, \beta_0, \theta_0, W_0)=(56, 1.1977\times10^{-5}, 37.765, 66.3407)$
...
$\theta_0\in (35, 40)$  $\omega_0\in(25, 30)$. The algorithm converged to
$(\theta_0, \omega_0)\approx (36.24, 27.46)$. As our solution failed in a little quantity to satisfy
...
Finally, the third step consisted in fixing the Fermionic-MW parameters but floating the ICs of the orbit with the genetic scheme, which gave us very similar to the FMW-MW case:
...
we have not performed any bayesian comparison between both models to determine which one is
more consistent with the data.
The corresponding orbit is displayed in the observable space in Figs.~\ref{fig:obs_pos}-\ref{fig:obs_vel}~
with a dotted green line where it can be noticed that both the Fermionic and NFW models
