...
of the Event Horizon Telescope~\citet{EHT_image} which suggest a compactness of the order of magnitude of 5 Schwarzschild radii.
...
Finally, we have shown that it is possible to find a unidimensional family of solutions parametrized by the fermion mass, which have the same halo that fits the GD-1 stream but they differ in the compactness of the central core. For the limiting case studied here ($m=360$~keV) we obtain a core radius of $r_c\approx5$~Schwarzschild radii where the transformation to this distance unit was done assuming a black hole mass of $m_{\rm{bh}}=4.075\times10^6$.
