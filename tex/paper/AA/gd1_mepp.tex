...
and the genetic algorithm specified in Sec.~\ref{sec:optimization}. It should be said that the
NFW-MW model was fitted by~\citet{2019MNRAS.486.2995M} with a set of Gaia DR2 data different from
the polynomials used here. Any point along the stream is equally good as an IC and the one
obtained is given approximately by $\alpha=149.44^{\circ}$, $\delta=36.81^{\circ}$, $D=7.92$ kpc,
