...
We model our Galaxy with a combination of the fermionic dark halo recently discribed, whose parameters will be determined in this work, plus a barionic component fixed and identical to the one in Model I of~\citet{2017A&A...598A..66P}.

As GD-1 is a dynamically cold stream, with its stars keeping a large degree of correlation, its almost
onedimensional distribution in phase-space can be well approximated with the orbit of its progenitor.
...
$\alpha$, $\delta$, $D_h$, $\mu_\alpha \cos\delta$, $\mu_\delta$, $v_h$, which are respectively, RA, declination, heliocentric distance, proper motion in $\alpha$, proper motion in $\delta$ and helicentric radial velocity.
The code makes use of the {\scshape{Astropy}} ecosystem~\citep{astropy:2022, astropy:2018, astropy:2013} in order to perform the following operations. First it transforms the initial condition to galactocentric coordinates assuming a Galactocentric reference frame with the sun at the position $\boldsymbol{x}_\odot=(-R_\odot,0,0)$ and a sun's velocity given by the sum of the circular velocity at the position of the sun and
the sun's peculiar velocity: $\boldsymbol{v}_\odot=(11.1, v_c(R_\odot)+12.24, 7.25)$. The cicular velocity is dependent on the model and the position and given by
...
The second step is to integrate the orbit forwards and backwards in time during a time interval of $\Delta t=0.2$ Gyr, starting in both cases from a given initial condition for the progenitor. In the next sections
we will explain how this initial condition was chosen for some simulations and fitted for others.
