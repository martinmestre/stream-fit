...
In one of the experiments we will consider an observable of a different nature of the stream
but related to the density concentration at the origin, of size smaller than $10^{-6}~\rm{kpc}$, characteristic of the fermionic halo distribution. By convention, the core radius is defined
...
Finally, we build interpolators of these variables that will be used, together with the observed data,
to evaluate the following $\chi^2$ stream function:
\begin{eqnarray}
   \label{chi2}
   \chi^2_{\rm{stream}} &=& \chi^2_{\phi_2} + \chi^2_{D}+ \chi^2_{v_h}+\chi^2_{\tilde{\mu}_\alpha}+ \chi^2_{\delta},\\
...
For some fits we will also consider the departature of the model from a mass constraint in the core
of the distribution:
\begin{equation}
   \chi^2_{\rm{core}}=\frac{(m-m_{\rm{core}})^2}{\sigma_m^2},
\end{equation}
usually testing with $\sigma_m$ equal to either 10\% or 50\% of the core mass, so
sometimes we will make use of the following compound function:
\begin{equation}
...
Our objective consists in fitting our MW model by minimizing the $\chi^2$ function given by Eq.~(\ref{chi2}).  In order to do so we use two optimization algorithms in this work.
