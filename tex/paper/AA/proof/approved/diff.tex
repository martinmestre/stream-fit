...
%DIF ADD approved.tex    Mon Jun  3 09:03:34 2024
...
\citep{1996ApJ...462..563N} }\hspace{0pt}%DIFAUXCMD
...
\DIFdel{6D }\DIFdelend \DIFaddbegin \DIFadd{5D }\DIFaddend track of the GD-1 stellar stream inside \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend MW with a fermionic dark matter core-halo distribution. At the same time, we aim to explain the dynamics of the S-cluster stars through the high-density fermion core and without assuming a central BH. Finally, we compare the virial mass of the Galaxy as predicted by the fermionic model with that obtained from recent \DIFdelbegin \textit{\DIFdel{Gaia}} %DIFAUXCMD
...
The core radius does not include the entire mass inside the innermost S-star pericenter we considered (S2) because the first maximum of the circular velocity corresponds to a shorter distance at which the core density-region is still falling. In Sec. \ref{sec:results} we obtain a DM mass inside the S2 star pericenter of \DIFdelbegin \DIFdel{$M(r_{peri-\rm{S2}}) = 4.03\times 10^6 M_\odot$}\DIFdelend \DIFaddbegin \DIFadd{$M(r_{\rm{peri-S2}}) = 4.03\times 10^6 M_\odot$}\DIFaddend , which agrees very well with the Schwarzschild BH mass constraints of $4.1\times 10^6 M_\odot$ and $3.97\times 10^6 M_\odot$ obtained from the same S2 star in \cite{2018A&A...615L..15G} and \cite{2019Sci...365..664D}, respectively.
...
\begin{align}
    ...
    \beta_0 &= \frac{kT_0}{mc^2}\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \label{rar_params}
\end{align}
\DIFdelbegin \DIFdel{We subtracted }\DIFdelend \DIFaddbegin \DIFadd{subtracting }\DIFaddend the rest-energy in the first two cases.
...
for \mbox{%DIFAUXCMD
\citet{Eilers_2019}}\hspace{0pt}%DIFAUXCMD
, rhombi (light-blue) for \mbox{%DIFAUXCMD
\citet{sofue_2020} }\hspace{0pt}%DIFAUXCMD
and
circles (black) for \mbox{%DIFAUXCMD
\citet{Jiao2023}}\hspace{0pt}%DIFAUXCMD
.
...
\cite{2010MNRAS.406..264W} }\hspace{0pt}%DIFAUXCMD
