...
# Observables
#
# GD-1 data from Ibata polynomials evaluated in a grid.
pol = IbaPoly()
...
#
# Mass estimates from Gibbons+14
barionic_mass = 0.9e11
r_data = np.array([50., 100.])
dark_mass_data = np.array([2.9e11-barionic_mass, 4.1e11-barionic_mass])
d_dark_mass_data = np.array([0.4e11, 0.7e11])
...
def chi2(w_0, ener_f, beta_0, ic):
    ...
    # Mass estimate from Sagittarius model (Gibbons et al.)
    halo = pot_list[3]
    dark_mass_mod = np.zeros(2)
    dark_mass_mod[0] = halo.mass_wrap(r_data[0])
    dark_mass_mod[1] = halo.mass_wrap(r_data[1])
    sum[5] = weight*np.sum((dark_mass_data-dark_mass_mod)**2 / d_dark_mass_data**2)

    print('Stream chi^2 =', np.sum(sum[0:5]), '  ---   Mass chi^2 =', sum[5]/weight)
    print('Total chi^2 =', np.sum(sum))
    print(' v_circ = ', v_circ)
    print(' theta_0 =', theta_0, ' W_0 =', W_0)
    print('scales =', scales)
    print('Virial radius =', halo.r_max, ' kpc')
    print('Virial mass =', halo.mass_wrap(halo.r_max)/1.e11, ' x10^11 M_sun')
    print('Halo mass(40kpc)=', halo.mass_wrap(40.)/1.e11, ' x10^11 M_sun')
    return np.sum(sum)
...
plt.legend()
plt.tight_layout()
fig.savefig("orbit_pot-slice-massSag_from_IbataPolysGaiaDR2-data.png")
# plt.show()
...
# plt.tight_layout()
fig.savefig("sky_pot-slice-massSag_from_IbataPolysGaiaDR2-data.png")
# plt.show()
...
plt.savefig('rotation_curve_pot-slice-massSag.png')
# plt.show()
