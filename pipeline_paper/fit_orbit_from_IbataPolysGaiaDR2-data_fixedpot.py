...
def orbit_model(alpha, delta, distance, mu_alpha_cosdelta, mu_delta, v_los, pot_list):
    """
    Orbit definition.
...
ic_input_file = "param_fit_I-M-GaiaDR2_to_MWPot2014wGalpy.txt"
w_ic = np.loadtxt(ic_input_file)
dw = np.abs(w_ic)*0.2
...
w_ic = np.loadtxt(ic_output_file)

print("w_ic=", w_ic)
chi2(w_ic, pot_list)

phi_1, phi_2, d_hel, mu_ra, mu_dec, v_hel, x, y, z, v_circ = orbit_model(w_ic[0], w_ic[1], w_ic[2], w_ic[3],
                                                                         w_ic[4], w_ic[5], pot_list)
