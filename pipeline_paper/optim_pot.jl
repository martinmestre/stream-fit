...
@everywhere begin
    ...
    function worker(i, sol_dir, m, ic, r☼, lb, ub, ng)
        println("Npiow at id:$(myid()), host:$(gethostname())")
        x₀ = 0.5*(lb+ub)
        p = (m, ic, r☼)
...
# Just for a pulished run for m=360 keV, in a small window
const lb_g = [[35.8, 27.0, 1.2e-5], [36., 27., 1.2e-5], [37., 28., 5.0e-5],
              [38., 29., 3.5e-4], [43.04, 29.50, 0.0025], [43., 29.6, 3.0e-3]]
const ub_g = [[36.3, 27.6, 1.3e-5], [40., 31., 1.0e-4], [41., 32., 1.0e-3],
              [42., 32., 3.0e-3], [43.07, 29.55, 0.0035], [47., 36., 1.0e-2]]
