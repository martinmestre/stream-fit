...
outfile = "solutions_stream_core_Metaheuristics.txt"
for ϵ in E
    global  θ₀, Δθ₀, βᵣ₀, x₀
    open(outfile, "a") do f
        # Core  χ²
        ...
        ub = [1.01θ₀, 1.01Δθ₀, 1.05βᵣ₀]
        info = Metaheuristics.Information(f_optimum = 30.47)
        prob = OptimizationProblem(χ²Full, x₀, p, ic=ic, r☼=r☼, lb=lb, ub=ub)
        sol = solve(prob, DE(information=info), use_initial=true, abstol=0.5e-5, reltol=5.0e-5)
        x₀ = sol.u
        χ² = sol.minimum
        θ₀ = x₀[1]
        Δθ₀ = x₀[2]
