...
# %%
df = DataFrame(CSV.File("solutions_stream_core_polished_with_GR_NLoptNelderMead.txt"; delim="  "))
...
df.chi2full = parse.(Float64, df.chi2full)
df.chi2stream = parse.(Float64, df.chi2stream)
println(df)
...
for i = 1:length(ϵ)
      ...
      ρ=ρ_f(r)
      df_plot = (x=r, y=ρ./(4π*r.^2), Energy=fill(labels[i],length(r)))
...
fig = Figure()
ag = draw!(fig[1,1], full_plt, axis=(xlabel=L"$r$ [kpc]",
            ylabel=L"$ρ$ [$M_⊙$/kpc³]",
